...
from lib.config.path import common_sql_path, global_env_path
...
def get_check_t_p(param):
    ...
    if res:
        ...
        if env_id == "grey" or env_id == "product":
            GlobalVar.MYSQL_AUTO_TEST.execute(
                Config(common_sql_path).read_config("pay_auto_test_info", "product_update_account").format(
                    token_new, Config(global_env_path).read_config("product_account", "account")))
