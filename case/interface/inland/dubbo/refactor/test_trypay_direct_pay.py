...
def test_create_direct_pay_positive(case):
    try:
        req, result = create_payorder_positive(case, partner_id=partner_id, directPay='ZHICHONG')
    finally:
        # 更新下发请求报文体到对应表格中
...
def test_create_direct_pay_negative(case):
    try:
        req, result = create_payorder_negative(case, partner_id=partner_id, directPay='ZHICHONG')
    finally:
        # 更新下发请求报文体到对应表格中
        case_file.update_req(case.name, req)
        # 更新到实际结果对应表格中
