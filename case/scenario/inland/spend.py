...
def spend(amount, vou_amount=round(random.uniform(0.02, 10), 2)):
    ...
    result = ExpendPay(int(amount+vou_amount*100), req.partner_id, req.interface_version,
                  req.app_version, req.notify_url).kb_voucher_spend(vou_info['vouId'], req_vou.vouType, int(vou_amount*100))
    ...
    FizAssert().assert_order_info(req.ssoid, pay_req_id, 0, int(amount + vou_amount * 100), amount, int(vou_amount*100), str(vou_info["vouId"]))
    """
        【6】. 检查trade_order表信息是否正确
    """
    FizAssert().assert_trade_order(req.ssoid, pay_req_id, 0, int(amount + vou_amount * 100), amount, int(vou_amount*100))
    """
        【7】. 检查通知表信息是否正确
    """
